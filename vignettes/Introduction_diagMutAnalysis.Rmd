...
For all functions used in `diagMutAnalysis` package a data set configured correctly needs to be used. Go to [Zhang J et al.] and find DCC data. The easiest way to use this package is to configure data into similar format as files denoted as simple_somatic_mutation.open.\<Project-code\>.tsv.gz. Without setting it to exactly as same as the suggested data format package is still fully suitable to be used. Need to follow function guidelines, and which object types to use.
